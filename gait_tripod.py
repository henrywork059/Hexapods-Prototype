\
# gait_tripod.py
# Tripod gait scheduler (I2C version) with:
#   - command latching (changes only take effect at phase boundaries)
#   - phase-boundary replanning (no mid-swing retarget)
#   - stance + swing trajectories from trajectory.py
#   - hip-local IK via ik.py
#
# Designed for your CURRENT setup:
#   - No world coordinates required
#   - Works purely with body frame + hip frames (relative coordinate motions)
...
class TripodGait:
    ...
    def __init__(self):
        # ---- tunables (can be overridden in config) ----
        self.period = float(getattr(config, "GAIT_PERIOD", 0.8))   # seconds per gait cycle
...
if __name__ == "__main__":
    ...
    gait.set_command(40.0, 0.0, 0.0)  # forward

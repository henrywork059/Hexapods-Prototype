\
# ik.py
# Hip-local IK for a 3DOF hexapod leg (I2C version).
#
...
#
# Output driver: servo.py (PCA9685 over I2C)
...
# Geometry from your robot config
...
def ik_xyz(x, y, z):
    """Solve IK for ONE leg in hip-local coordinates (mm).

    Returns dict with logical angles: coxa, femur, tibia (deg).
    Knee-up branch (leg looks like /\\).
    """
    x = float(x)
    y = float(y)
    z = float(z)
...
def move_all_legs_xyz(targets, t_ms=None):
    ...
    targets: length 6 list of (x,y,z) hip-local for each leg.
    """
    if len(targets) != 6:

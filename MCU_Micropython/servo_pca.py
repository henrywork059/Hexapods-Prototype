# servo_pca.py
# PCA9685 I2C PWM servo driver with a servo_uart-compatible API.
#
# Exports:
#   init()
...
#
# Notes:
# - PCA9685 does NOT have built-in motion timing. If t_ms is provided,
#   this module interpolates in software with small steps (blocking).
# - Angles are absolute servo angles in degrees (0..180 typical).
#   IK should already apply inversion/offset/clamps before calling this driver.
...
class PCA9685:
    ...
    def set_freq(self, freq_hz):
        ...
        # Totem pole output recommended for servo signal
...
def _set_logical_channel(ch, angle_deg):
    ...
    if board_idx >= len(_pcas):
        raise ValueError("Channel %d mapped to board %d, but only %d board(s) configured" %
...
def set_servo(ch, angle_deg, t_ms=None):
    ...
    steps = max(1, int(t_ms) // 20)  # ~50Hz stepping
